...
export const dbEngine = {
  ...
  insertReceiptStatus: (receipt: SQL_ReceiptStatus) => {
    ...
    // Ensure exit_verification is set to false if not provided
    const receiptWithExitVerification = {
      ...receipt,
      exit_verification: receipt.exit_verification ?? false
    };
    TABLE_RECEIPT_STATUS_REGISTRY.push(receiptWithExitVerification);
    saveReceiptTable(TABLE_RECEIPT_STATUS_REGISTRY); // Commit to DB
    console.log(`[SQL INSERT] INTO receipt_status_registry VALUES ('${receipt.receipt_number}', '${receipt.store_id}', ${receipt.total_amount}, '${receipt.payment_status}', ${receiptWithExitVerification.exit_verification})`);
  },
  ...
  selectReceipt: (receiptNumber: string): SQL_ReceiptStatus | null => {
    ...
    const receipt = TABLE_RECEIPT_STATUS_REGISTRY.find(r => r.receipt_number === receiptNumber);
    if (receipt) {
      // Ensure exit_verification exists for old records
      return {
        ...receipt,
        exit_verification: receipt.exit_verification ?? false
      };
  ...
  // Simulates: UPDATE receipt_status SET exit_verification = true WHERE receipt_number = ?
  updateExitVerification: (receiptNumber: string): boolean => {
    TABLE_RECEIPT_STATUS_REGISTRY = loadReceiptTable(); // Refresh from DB
    const index = TABLE_RECEIPT_STATUS_REGISTRY.findIndex(r => r.receipt_number === receiptNumber);
    
    if (index !== -1) {
      TABLE_RECEIPT_STATUS_REGISTRY[index].exit_verification = true;
      saveReceiptTable(TABLE_RECEIPT_STATUS_REGISTRY); // Commit to DB
      console.log(`[SQL UPDATE] receipt_status_registry SET exit_verification=true WHERE receipt_number='${receiptNumber}'`);
      return true;
    }
    return false;
  }
